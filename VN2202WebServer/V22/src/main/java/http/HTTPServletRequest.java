...
public class HTTPServletRequest {
    ...
    private void parseContent() throws IOException {
        ...
        // 1. 通过判断content-length判断是否有消息正文
        if (headers.containsKey("Content-Length")) {
            int content_length = Integer.parseInt(headers.get("Content-Length"));
            System.out.println("消息正文长度：" + content_length);
            byte[] content_data = new byte[content_length];
            InputStream in = socket.getInputStream();
            in.read(content_data);
            String content_type = headers.get("Content_Type");
            if ("application/x-www-form-urlencoded".equals(content_type)){
                String line = new String(content_data,StandardCharsets.ISO_8859_1);

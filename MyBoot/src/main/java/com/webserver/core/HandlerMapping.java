...
public class HandlerMapping {
    ...
    private static void initMapping(){
        try {
            File dir = new File(
                    WebSeverApplication.TARGET.getClassLoader().getResource("./controller").toURI()
            );
            File[] files = dir.listFiles(f->f.getName().endsWith(".class"));
            for (File file : files) {
                String fileName = file.getName();
                String className = fileName.substring(0,fileName.indexOf("."));
                Class cls = Class.forName(WebSeverApplication.TARGET.getPackage().getName() + ".controller."+className);
